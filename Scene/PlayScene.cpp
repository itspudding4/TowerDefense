...
void PlayScene::Update(float deltaTime) {
    ...
    for (int i = 0; i < SpeedMult; i++) {
        ...
        switch (current.first) {
            ...
            case 2:
                EnemyGroup->AddNewObject(enemy = new ga_planeEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
                break;
            //     ...
            case 3:
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
    ...
    while (!que.empty()) {
        ...
        for (auto &dir : directions) {
            Engine::Point adj(p.x + dir.x, p.y + dir.y);

            // Check boundaries and walkability first
            if (adj.x < 0 || adj.x >= MapWidth || adj.y < 0 || adj.y >= MapHeight)
                continue;
            if (mapState[adj.y][adj.x] != TILE_DIRT || map[adj.y][adj.x] != -1)
                continue;

            map[adj.y][adj.x] = map[p.y][p.x] + 1;
            que.push(adj);
        }
